...
import { AgencyDomainTlsStatus, AgencyDomainType, AgencyDomainVerificationStatus } from '@prisma/client';
import { ensureDefaultAgency } from '../../lib/currentAgency';
...
function normalizeHost(input: string): string {
  return String(input || '').trim().toLowerCase().replace(/^https?:\/\//i, '').replace(/\/$/, '').replace(/:\d+$/, '');
}

function buildAgencySubdomainHost(slug: string): string {
  const platformRootDomain = normalizeHost(process.env.PLATFORM_ROOT_DOMAIN || process.env.NEXT_PUBLIC_PLATFORM_ROOT_DOMAIN || 'localhost');
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  await prisma.agencyDomain.deleteMany();
  await prisma.agency.deleteMany();

  const agency = await ensureDefaultAgency();
  await prisma.agencyDomain.upsert({
    where: { host: buildAgencySubdomainHost(agency.slug) },
    update: {
      agencyId: agency.id,
    ...
    create: {
      agencyId: agency.id,
      host: buildAgencySubdomainHost(agency.slug),
      type: AgencyDomainType.SUBDOMAIN,
      isActive: true,
      isPrimary: true,
      isVerified: true,
  ...
  // Crea categorías
  const beach = await prisma.category.create({ data: { agencyId: agency.id, name: 'Playa' } });
  const mountain = await prisma.category.create({ data: { agencyId: agency.id, name: 'Montana' } });
  const city = await prisma.category.create({ data: { agencyId: agency.id, name: 'Ciudad' } });
  const adventure = await prisma.category.create({ data: { agencyId: agency.id, name: 'Aventura' } });
  const nature = await prisma.category.create({ data: { agencyId: agency.id, name: 'Naturaleza' } });
