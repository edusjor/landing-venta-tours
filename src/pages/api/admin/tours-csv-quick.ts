...
async function buildUniqueTourSlug(agencyId: number, value: unknown, excludeId?: number): Promise<string> {
...
async function importQuickCsv(agencyId: number, csvText: string) {
  ...
  for (const group of groups) {
    ...
    const existingTour = Number.isFinite(externalNumericId) && externalNumericId > 0
      ? await prisma.tour.findFirst({
          where: { id: Math.floor(externalNumericId), agencyId },
    ...
    if (existingTour) {
      const nextSlug = await buildUniqueTourSlug(agencyId, group.title, existingTour.id);
    ...
    const nextSlug = await buildUniqueTourSlug(agencyId, group.title);
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      const summary = await importQuickCsv(agencyId, csvText);
