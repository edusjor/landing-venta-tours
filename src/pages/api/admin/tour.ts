...
type SlugDatabase = {
  tour: {
    findFirst: (args: { where: { agencyId: number; slug: string; id?: { not: number } } }) => Promise<{ id: number } | null>;
  };
};
...
async function buildUniqueTourSlug(db: SlugDatabase, agencyId: number, value: unknown, excludeId?: number): Promise<string> {
...
async function resolveCategoryId(
  db: {
    category: {
      findFirst: (args: {
        where: { agencyId: number; id?: number; name?: string };
        select: { id: true };
      }) => Promise<{ id: number } | null>;
      create: (args: {
        data: { agencyId: number; name: string };
        select: { id: true };
      }) => Promise<{ id: number }>;
    };
  },
  ...
  if (Number.isFinite(candidateCategoryId) && candidateCategoryId > 0) {
    const existing = await db.category.findFirst({
      where: { agencyId, id: candidateCategoryId },
      select: { id: true },
    });
  ...
  const fallbackCategory = await db.category.findFirst({
    where: { agencyId, name: 'Sin categoría' },
    select: { id: true },
  });
  ...
  const createdFallbackCategory = await db.category.create({
    data: { agencyId, name: 'Sin categoría' },
    select: { id: true },
  });

  return createdFallbackCategory.id;
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      return res.status(500).json({ error: 'No se pudieron cargar los tours.', detail });
    }
  }
  ...
  try {
    if (req.method === 'POST') {
      const nextCategoryId = await resolveCategoryId(prisma, agencyId, parsedCategoryId);
      const nextSlug = await buildUniqueTourSlug(prisma, agencyId, requestedSlug || title);
    ...
    const existingScopedTour = await prisma.tour.findFirst({
      where: { id, agencyId },
      select: { id: true },
    });

    if (!existingScopedTour) {
      return res.status(404).json({ error: 'Tour no encontrado' });
    }

    if (req.method === 'DELETE') {
      await prisma.$transaction(async (tx) => {
        await tx.reservation.deleteMany({ where: { tourId: existingScopedTour.id } });
        await tx.availability.deleteMany({ where: { tourId: existingScopedTour.id } });
        await tx.tour.delete({ where: { id: existingScopedTour.id } });
      });
    ...
    if (req.method === 'PATCH') {
      ...
      const updatedTour = await prisma.tour.update({
        where: { id: existingScopedTour.id },
        data: patchData,
        include: { availability: true, category: true },
      });
    ...
    const tour = await prisma.$transaction(async (tx) => {
      ...
      const nextCategoryId = await resolveCategoryId(tx, agencyId, parsedCategoryId);
      const effectiveStatus = toValidStatus(status) ?? existingTour.status;
      const nextSlug = requestedSlug
        ? await buildUniqueTourSlug(tx, agencyId, requestedSlug, id)
        : effectiveStatus === 'BORRADOR'
          ? await buildUniqueTourSlug(tx, agencyId, title || existingTour.slug || `tour-${id}`, id)
          : existingTour.slug;
