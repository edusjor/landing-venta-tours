...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (publicAgency && isPublicStorefrontBlocked(publicAgency)) {
  ...
  const toEmail = publicAgency?.email || process.env.CONTACT_TO_EMAIL || smtpUser || '';
  const agencyName = publicAgency?.name || 'Agencia';
  ...
  if (!toEmail) {
    return res.status(500).json({
      error: 'No hay correo de recepción configurado para esta agencia.',
    });
  }
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      to: toEmail,
      replyTo: email,
      subject: `[Contacto Web - ${agencyName}] ${asunto}`,
      text: [
        `Nuevo mensaje de contacto (${agencyName})`,
        '',
        `Nombre: ${nombre}`,
        `Correo: ${email}`,
        `Teléfono: ${telefono}`,
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:720px;margin:0 auto;border-collapse:collapse;">
            <tr>
              <td style="background:linear-gradient(135deg,#065f46 0%,#0f766e 100%);padding:22px 24px;border-radius:14px 14px 0 0;">
                <p style="margin:0 0 6px 0;font-size:12px;letter-spacing:.08em;text-transform:uppercase;font-weight:700;color:#d1fae5;">${agencyName}</p>
