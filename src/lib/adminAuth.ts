...
function normalizeSessionAgencySlug(value: unknown): string | undefined {
  const normalized = String(value ?? '').trim().toLowerCase();
  return normalized || undefined;
}

export function createAdminSessionToken(username: string, agencySlug?: string | null): string {
  const payload: AdminSessionPayload = {
    username,
    exp: Date.now() + SESSION_TTL_SECONDS * 1000,
    agencySlug: normalizeSessionAgencySlug(agencySlug),
  };
...
export function verifyAdminSessionToken(token: string): AdminSessionState {
  ...
  try {
    ...
    return {
      ...
      agencySlug: normalizeSessionAgencySlug(parsed.agencySlug),
    };
  } catch {
    return { ok: false };
  }
...
export function getAdminSessionFromRequest(req: NextApiRequest): AdminSessionState {
...
export function requireAdminSession(req: NextApiRequest, res: NextApiResponse): boolean {
  ...
  if (session.ok) {
    ...
    if (session.username) {
      setAdminSessionCookie(res, createAdminSessionToken(session.username, session.agencySlug));
    }
    return true;
  }
