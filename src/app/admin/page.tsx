...
function AdminPageContent() {
  ...
  const loadData = useCallback(async () => {
    ...
    Promise.all([fetch("/api/admin/category"), fetch("/api/admin/tour")])
      .then(async ([categoriesRes, toursRes]) => {
        if (categoriesRes.status === 401 || toursRes.status === 401) {
          throw new Error("UNAUTHORIZED");
        }

        const [apiCategories, apiTours] = await Promise.all([
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/admin/auth", {
        ...
        body: JSON.stringify({ username: loginUser, password: loginPass, agencySlug: loginAgencySlug }),
      });
        ...
        <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-lg shadow-slate-300/40">
          ...
          <form className="mt-5 space-y-3" onSubmit={handleLogin}>
            ...
            <label className="block text-sm font-bold text-slate-700">
              ...
              <input className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2" placeholder="mi-agencia" value={loginAgencySlug} onChange={(e) => setLoginAgencySlug(e.target.value)} />
            </label>
          ...
          <p className="mt-3 text-xs text-slate-500">Puedes ingresar con credencial global admin o con cuenta owner de agencia.</p>
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => router.push("/admin/resumen")}
                  className="whitespace-nowrap rounded-lg border border-emerald-300 bg-white px-3 py-2 text-xs font-semibold text-emerald-700 hover:bg-emerald-50 md:text-sm"
                >
                  Resumen agencia
                </button>
                <button
                ...
                <button
                  ...
                  className="whitespace-nowrap rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 md:text-sm"
                >
                ...
                <button
                  ...
                  className="whitespace-nowrap rounded-lg border border-violet-300 bg-white px-3 py-2 text-xs font-semibold text-violet-700 hover:bg-violet-50 md:text-sm"
                >
                  Pagos manuales
                </button>
