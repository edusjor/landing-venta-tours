...
export function buildLegalMetadata(config: LegalPageConfig): Metadata {
  const legalBrandName = String(
    process.env.PLATFORM_LEGAL_BRAND_NAME
    ?? process.env.NEXT_PUBLIC_PLATFORM_NAME
    ?? process.env.PLATFORM_DEFAULT_AGENCY_NAME
    ?? process.env.DEFAULT_AGENCY_NAME
    ?? 'Plataforma de Tours',
  ).trim() || 'Plataforma de Tours';

  return {
    title: `${config.title} | ${legalBrandName}`,
    description:
      config.description ??
      `Consulta ${config.title.toLowerCase()} de ${legalBrandName}.`,
  };
}
