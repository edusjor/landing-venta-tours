...
import { getClientPublicAgencySlug } from "../../../../lib/publicAgencyClient";
import {
...
function getScopedLocalToursKey(agencySlug: string): string {
  const normalizedAgencySlug = String(agencySlug ?? "").trim();
  return normalizedAgencySlug ? `${LOCAL_TOURS_KEY}:${normalizedAgencySlug}` : LOCAL_TOURS_KEY;
}
...
interface TourLite {
  ...
  tourPackages: TourPackage[];
  availabilityConfig?: AvailabilityConfig;
  paymentOptions: PublicAgencyPaymentOptions;
}
...
function normalizePublicPaymentOptions(input: unknown): PublicAgencyPaymentOptions {
  if (!input || typeof input !== "object") return DEFAULT_PUBLIC_PAYMENT_OPTIONS;

  const source = input as Partial<PublicAgencyPaymentOptions>;
  const paymentModeRaw = String(source.paymentMode ?? DEFAULT_PUBLIC_PAYMENT_OPTIONS.paymentMode).trim().toUpperCase();
  const paymentMode = paymentModeRaw === "GATEWAY" || paymentModeRaw === "MANUAL" || paymentModeRaw === "BOTH"
    ? paymentModeRaw
    : DEFAULT_PUBLIC_PAYMENT_OPTIONS.paymentMode;

  let allowsGateway = Boolean(source.allowsGateway);
  let allowsManual = Boolean(source.allowsManual);
  ...
  if (!allowsGateway && !allowsManual) {
    allowsGateway = DEFAULT_PUBLIC_PAYMENT_OPTIONS.allowsGateway;
    allowsManual = DEFAULT_PUBLIC_PAYMENT_OPTIONS.allowsManual;
  }

  return {
    paymentMode,
    gatewayProvider: normalizeOptionalText(source.gatewayProvider) || DEFAULT_PUBLIC_PAYMENT_OPTIONS.gatewayProvider,
    allowsGateway,
    allowsManual,
    manualPaymentInstructions: normalizeOptionalText(source.manualPaymentInstructions),
    bankAccountName: normalizeOptionalText(source.bankAccountName),
    bankAccountNumber: normalizeOptionalText(source.bankAccountNumber),
    bankAccountIban: normalizeOptionalText(source.bankAccountIban),
    sinpeMobile: normalizeOptionalText(source.sinpeMobile),
  };
}

function getAvailablePaymentMethods(options: PublicAgencyPaymentOptions): string[] {
  const methods: string[] = [];
  if (options.allowsGateway) methods.push(PAYMENT_METHOD_GATEWAY);
  if (options.allowsManual) methods.push(PAYMENT_METHOD_MANUAL);
  return methods.length ? methods : [PAYMENT_METHOD_GATEWAY];
}
...
function getLocalReservationSeed(tourSlug: string, agencySlug: string): {
  tour: TourLite | null;
  availability: Availability[];
  availabilityConfig: AvailabilityConfig;
  ...
  const localTours = safeParse<LocalTourAvailability[]>(localStorage.getItem(getScopedLocalToursKey(agencySlug)), []);
  ...
  return {
    tour: localTour
      ? {
          ...
          availabilityConfig,
          paymentOptions: DEFAULT_PUBLIC_PAYMENT_OPTIONS,
        }
      : null,
    availability,
    availabilityConfig,
...
export default function ReservarPage() {
  ...
  const agencyFromQuery = getClientPublicAgencySlug(searchParams?.get("agency"));

  return (
    <ReservarPageContent
      key={`${tourSlug}:${agencyFromQuery ?? "default-agency"}:${packageFromQuery ?? "default"}:${dateFromQuery ?? "no-date"}`}
      tourSlug={tourSlug}
      agencySlug={agencyFromQuery}
...
function ReservarPageContent({
  ...
  const [payMethod, setPayMethod] = useState(PAYMENT_METHOD_GATEWAY);
  ...
  const [isUploadingManualPaymentProof, setIsUploadingManualPaymentProof] = useState(false);
  ...
  const [demoCardHolder, setDemoCardHolder] = useState("MARIA GONZALEZ");
  const [demoCardNumber, setDemoCardNumber] = useState("4242 4242 4242 4242");
  const [demoCardExpiry, setDemoCardExpiry] = useState("12/30");
  ...
  useEffect(() => {
    ...
    const normalizedAgencySlug = String(agencySlug ?? "").trim();
    const localData = getLocalReservationSeed(tourSlug, normalizedAgencySlug);
    if (localData.tour) {
    ...
    fetch(`/api/tour?slug=${encodeURIComponent(tourSlug)}${normalizedAgencySlug ? `&agency=${encodeURIComponent(normalizedAgencySlug)}` : ""}`)
      .then((res) => res.json())
      .then((data) => {
        if (data?.id) {
          ...
          setTour({
            ...
            paymentOptions: normalizePublicPaymentOptions(data.paymentOptions),
          });
  ...
  }, [agencySlug, normalizedDateFromQuery, todayDateKey, tourSlug]);
  ...
  const paymentOptions = useMemo(
    () => normalizePublicPaymentOptions(tour?.paymentOptions),
    [tour?.paymentOptions],
  );

  const availablePaymentMethods = useMemo(
    () => getAvailablePaymentMethods(paymentOptions),
    [paymentOptions],
  );

  useEffect(() => {
    if (!availablePaymentMethods.includes(payMethod)) {
      setPayMethod(availablePaymentMethods[0] ?? PAYMENT_METHOD_GATEWAY);
      setManualPaymentProofFile(null);
      setManualPaymentProofUrl("");
    }
  }, [availablePaymentMethods, payMethod]);
  ...
  const canContinueToPay =
    ...
    && (isDemoSiteRoute
      ? true
      : Boolean(name.trim() && lastName.trim() && email.trim() && phoneCountryDialCode.trim() && phone.trim()));
  const isManualPaymentMethod = normalizePaymentMethod(payMethod) === PAYMENT_METHOD_MANUAL;
  ...
  const openPaymentStep = () => {
    ...
    if (!canContinueToPay) {
      setStepError({
        step: "pago",
        message: isDemoSiteRoute
          ? "Para esta demo puedes continuar sin llenar contacto."
          : "Completa tus datos de contacto para poder abrir el paso de pago.",
      });
      return;
    }
    setStepError(null);
  ...
  const navigateToConfirmation = (input: {
    ...
    status: "confirmed" | "pending" | "pending_payment" | "payment_review" | "rejected";
    paymentMethod: string;
    message: string;
  }) => {
    setIsRedirectingToConfirmation(true);
    const isDemoSiteRoute = pathname === "/demo-sitio/tours" || pathname.startsWith("/demo-sitio/tours/");
    const confirmationBasePath = isDemoSiteRoute ? "/demo-sitio/reserva-confirmada" : "/reserva-confirmada";
    ...
    router.push(`${confirmationBasePath}?${query.toString()}`);
  };
  ...
  const handleReserve = async (e: React.FormEvent) => {
    ...
    let uploadedManualPaymentProofUrl = manualPaymentProofUrl;

    if (isManualPaymentMethod && !uploadedManualPaymentProofUrl && manualPaymentProofFile) {
      try {
        setIsUploadingManualPaymentProof(true);
        setStatus("Subiendo comprobante de pago manual...");

        const formData = new FormData();
        formData.append("receipt", manualPaymentProofFile);
        ...
        if (!uploadRes.ok) {
          setStatus(uploadPayload?.error ? `No se pudo subir el comprobante: ${uploadPayload.error}` : "No se pudo subir el comprobante de pago manual.");
          return;
        }

        uploadedManualPaymentProofUrl = String(uploadPayload?.url ?? "").trim();
        if (!uploadedManualPaymentProofUrl) {
          setStatus("No se recibió una URL válida del comprobante de pago manual.");
          return;
        }

        setManualPaymentProofUrl(uploadedManualPaymentProofUrl);
      } finally {
        setIsUploadingManualPaymentProof(false);
      }
    }
    ...
    try {
      ...
      const fallbackDemoEmail = "cliente.demo@example.com";
      const fallbackDemoDialCode = "+506";
      const fallbackDemoPhone = "70000000";

      const safeName = name.trim() || (isDemoSiteRoute ? fallbackDemoName : "");
      const safeLastName = lastName.trim() || (isDemoSiteRoute ? fallbackDemoLastName : "");
      const safeEmail = email.trim() || (isDemoSiteRoute ? fallbackDemoEmail : "");
      const safeDialCode = phoneCountryDialCode.trim() || (isDemoSiteRoute ? fallbackDemoDialCode : "");
      const safePhoneNumber = phone.trim() || (isDemoSiteRoute ? fallbackDemoPhone : "");
      const fullPhone = `${safeDialCode} ${safePhoneNumber}`.trim();
      ...
      const res = await fetch("/api/reservar", {
        ...
        body: JSON.stringify({
          ...
          manualPaymentProofUrl: uploadedManualPaymentProofUrl || null,
          sinpeReceiptUrl: uploadedManualPaymentProofUrl || null,
      ...
      if (!payload?.requiresPayment) {
        ...
        const nextMessage =
          ...
          (isManualPaymentMethod
            ? "Recibimos tu solicitud de pago manual. Te contactaremos cuando sea revisada."
            : "Reserva confirmada. Te enviamos el detalle por correo.");

        if (isManualPaymentMethod) {
          const nextStatusRaw = String(payload?.status ?? payload?.reservationStatus ?? "").trim().toLowerCase();
          const nextStatus =
            nextStatusRaw === "payment_review"
              ? "payment_review"
              : nextStatusRaw === "rejected"
                ? "rejected"
      ...
      if (!window.onvo?.pay) {
        setStatus("No se pudo inicializar el checkout de tarjeta. Recarga la pagina e intenta nuevamente.");
        return;
      }
      ...
      setStatus("Completa tu pago en el formulario de tarjeta para finalizar la reserva.");
    } catch {
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-8">
      <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 sm:text-4xl">Reserva tu tour</h1>
            ...
            <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
              <div className="mb-3 flex items-center gap-2">
                <button
                  ...
                  className="rounded-md border border-slate-300 bg-white px-2 py-1 text-xs font-bold text-slate-700 disabled:cursor-not-allowed disabled:opacity-40 sm:px-3"
                >
                  Prev
                </button>
                <p className="flex-1 truncate px-1 text-center text-sm font-extrabold text-slate-800 sm:text-lg">
                  {visibleMonth
          ...
          <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            {step === "contacto" && (
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                {isDemoSiteRoute ? (
                  <p className="md:col-span-2 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-xs font-semibold text-emerald-800">
                    En esta demo, los datos de contacto son opcionales para continuar al pago.
                  </p>
                ) : null}
                <div>
                ...
                >
                  {isDemoSiteRoute ? "Continuar a pago (sin contacto obligatorio)" : "Continuar a pago"}
                </button>
              </div>
            )}
              ...
              <form onSubmit={handleReserve} className="mt-4 space-y-3">
                ...
                {isManualPaymentMethod ? (
                  <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
                    <p className="text-sm font-extrabold text-amber-900">Pago manual</p>
                    {paymentOptions.manualPaymentInstructions ? (
                      <p className="mt-1 whitespace-pre-line text-sm text-amber-900">{paymentOptions.manualPaymentInstructions}</p>
                    ) : (
                      <p className="mt-1 text-sm text-amber-900">Completa la transferencia o depósito y te confirmaremos después de revisar el pago.</p>
                    )}
                    <p className="mt-1 text-sm font-semibold text-slate-800">
                      Total a transferir: <span className="font-extrabold text-emerald-800">{formatCurrencyUSD(total)}</span>
                    </p>

                    {(paymentOptions.bankAccountName || paymentOptions.bankAccountNumber || paymentOptions.bankAccountIban || paymentOptions.sinpeMobile) ? (
                      <div className="mt-3 rounded-lg border border-amber-300 bg-white p-3 text-sm text-slate-700">
                        <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Datos para pago manual</p>
                        {paymentOptions.bankAccountName ? <p className="mt-1"><span className="font-semibold">Cuenta a nombre de:</span> {paymentOptions.bankAccountName}</p> : null}
                        {paymentOptions.bankAccountNumber ? <p><span className="font-semibold">Numero de cuenta:</span> {paymentOptions.bankAccountNumber}</p> : null}
                        {paymentOptions.bankAccountIban ? <p><span className="font-semibold">IBAN:</span> {paymentOptions.bankAccountIban}</p> : null}
                        {paymentOptions.sinpeMobile ? <p><span className="font-semibold">SINPE movil:</span> {paymentOptions.sinpeMobile}</p> : null}
                      </div>
                    ) : null}

                    <div className="mt-3">
                      <label className="mb-1 block text-xs font-bold uppercase tracking-wide text-slate-600">Subir comprobante (opcional)</label>
                      <input
                        type="file"
                        accept="image/*"
                        onChange={(e) => {
                          const nextFile = e.target.files?.[0] ?? null;
                          setManualPaymentProofFile(nextFile);
                          setManualPaymentProofUrl("");
                        }}
                        className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm"
                      />
                      {manualPaymentProofFile ? (
                        <p className="mt-2 text-xs font-semibold text-emerald-700">Comprobante listo: {manualPaymentProofFile.name}</p>
                      ) : manualPaymentProofUrl ? (
                        <p className="mt-2 text-xs font-semibold text-emerald-700">Ya subiste un comprobante en esta sesión.</p>
                      ) : null}
                    </div>
                  </div>
                ) : null}
                ...
                {availablePaymentMethods.length === 0 ? (
                  <p className="rounded-lg border border-rose-200 bg-rose-50 p-3 text-sm font-semibold text-rose-700">
                    Esta agencia no tiene métodos de pago disponibles en este momento.
                  </p>
                ) : null}
                  ...
                  {isRedirectingToConfirmation
                    ...
                    : isUploadingManualPaymentProof
                      ...
                      : isCreatingPayment
                        ? "Preparando pago..."
                        : isManualPaymentMethod
                          ? "Enviar reserva con pago manual"
                          : "Confirmar reserva y pagar"}
                </button>

                {!isManualPaymentMethod ? (
                  <div className="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-3">
                    <p className="text-xs font-bold uppercase tracking-wide text-slate-600">Datos de tarjeta (demo)</p>
                    <div className="mt-3 grid gap-3 md:grid-cols-2">
                      <label className="grid gap-1 text-xs font-semibold text-slate-700 md:col-span-2">
                        Titular
                        <input
                          value={demoCardHolder}
                          onChange={(e) => setDemoCardHolder(e.target.value)}
                    ...
                    <div id="onvo-checkout-container" className="mt-4 hidden rounded-lg border border-slate-200 bg-slate-50 p-3" />
                  </div>
                ) : null}
              </form>
            )}
        ...
        <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:sticky lg:top-6">
          ...
          <div className="mt-4 flex gap-3 rounded-xl border border-slate-200 p-3">
            ...
            <div className="min-w-0">
              <p className="line-clamp-2 text-sm font-extrabold text-slate-900">{tour.title}</p>
              ...
              <p className="text-xs text-slate-600">{normalizeTime24(effectiveSelectedTime) ? formatTimeLabel(effectiveSelectedTime) : effectiveSelectedTime}</p>
              <p className="text-xs text-slate-600">{totalPeople} pers.</p>
            </div>
          </div>
