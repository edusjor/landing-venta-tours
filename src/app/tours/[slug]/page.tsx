...
function getLocalTourByRouteParam(routeParam: string, agencySlug: string): Tour | null {
  const localTours = safeParse<Partial<Tour>[]>(localStorage.getItem(getScopedLocalToursKey(agencySlug)), []);
...
export default function TourDetailPage() {
  ...
  useEffect(() => {
    ...
    const remoteUrl = isNumericRoute
      ? `/api/tour?id=${encodeURIComponent(tourSlug)}${activeAgencySlug ? `&agency=${encodeURIComponent(activeAgencySlug)}` : ""}`
      : `/api/tour?slug=${encodeURIComponent(tourSlug)}${activeAgencySlug ? `&agency=${encodeURIComponent(activeAgencySlug)}` : ""}`;
    ...
    fetch(remoteUrl)
      ...
      .then((data) => {
        ...
        if (normalizedRemote && (!activeAgencySlug || normalizedRemote.agency?.slug === activeAgencySlug)) {
          setTour(normalizedRemote);
          return;
        }

        const localTour = getLocalTourByRouteParam(tourSlug, activeAgencySlug);
        if (localTour) {
          setTour(localTour);
          return;
        }
  ...
  if (selectedPackage) reserveQuery.set("package", selectedPackage.id);
  if (selectedAvailabilityDate) reserveQuery.set("date", selectedAvailabilityDate);
  if (activeAgencySlug) reserveQuery.set("agency", activeAgencySlug);
  const isDemoSiteRoute = pathname === "/demo-sitio/tours" || pathname.startsWith("/demo-sitio/tours/");
  const toursBasePath = isDemoSiteRoute ? "/demo-sitio/tours" : "/tours";
  const reserveHref = `${toursBasePath}/${encodeURIComponent(routeParam)}/reservar${reserveQuery.toString() ? `?${reserveQuery.toString()}` : ""}`;
