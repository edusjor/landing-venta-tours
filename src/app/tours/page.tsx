...
export default function ToursPage() {
  ...
  const localToursKey = getScopedLocalToursKey(activeAgencySlug);
  ...
  const reloadToursData = useCallback(() => {
    ...
    const localTours = safeParse<Tour[]>(localStorage.getItem(localToursKey), []);
    const agencyQuery = activeAgencySlug ? `?agency=${encodeURIComponent(activeAgencySlug)}` : "";

    Promise.all([fetch(`/api/tours${agencyQuery}`).then((res) => res.json()), fetch(`/api/categories${agencyQuery}`).then((res) => res.json())])
      .then(([tourData, categoryData]) => {
        ...
        localStorage.setItem(localToursKey, JSON.stringify(remoteTours));
      })
      .catch(() => {
        setTours(localTours);
        setCategories([]);
  ...
  }, [activeAgencySlug, localToursKey]);
  ...
  useEffect(() => {
    ...
    const onStorage = (event: StorageEvent) => {
      if (event.key === localToursKey || event.key === FILTER_CONFIG_KEY || event.key === "toursDataVersion") {
        reloadToursData();
      }
    };
  ...
  }, [activeAgencySlug, localToursKey, reloadToursData]);
              ...
              <article key={tour.id} className="flex flex-col overflow-hidden rounded-2xl bg-white shadow-xl shadow-slate-300/40">
                ...
                <div className="flex flex-1 flex-col p-5">
                  ...
                  <div className="mt-auto flex items-center justify-between gap-3 pt-4">
                    ...
                    <Link
                      href={activeAgencySlug
                        ? `${toursBasePath}/${encodeURIComponent(getTourRouteParam(tour))}?agency=${encodeURIComponent(activeAgencySlug)}`
                        : `${toursBasePath}/${encodeURIComponent(getTourRouteParam(tour))}`}
